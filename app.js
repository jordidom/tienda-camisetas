...
function createWhatsAppMessage(total) {
  const lines = cart.map(item =>
    `- ${item.title} | Cantidad: ${item.quantity} | Precio unidad: ${formatPrice(item.price)}`
  );

  return `Hola, quiero hacer este pedido:%0A%0A${lines.join("%0A")}%0A%0ATotal: ${formatPrice(total)}%0A%0APago por Bizum.`;
}
    ...
    <div class="cart-item">
      <div class="cart-item-top">
        <div>
          ...
          <p class="cart-item-meta">${formatPrice(item.price)} · ${item.team}</p>
